...
Edid.prototype.parse = function () {
  ...
  this.manufactureDate =
    this.getManufactureWeek() + '/' + this.getManufactureYear();

  this.edidVersion = this.getEdidVersion() + '.' + this.getEdidRevision();
  ...
  for (var extIndex = 0; extIndex < this.numberOfExtensions; extIndex++) {
    ...
    // Parse DTDs
    this.exts[extIndex].dtds = this.getExtDtds(
      extIndex,
      this.exts[extIndex].dtdStart
    );
    // Get Checkum
...
Edid.prototype.getBasicDisplayParams = function () {
  ...
  if (this.edidData[VIDEO_IN_PARAMS_BITMAP] & DIGITAL_INPUT) {
    ...
    bdp.vsyncSerrated =
      this.edidData[VIDEO_IN_PARAMS_BITMAP] & VSYNC_SERRATED_MASK
        ? true
        : false;
  }
  ...
  bdp.dpmsStandby =
    this.edidData[SUPPORTED_FEATURES_BITMAP] & DPMS_STANDBY ? true : false;
  var DPMS_SUSPEND = 0x40;
  bdp.dpmsSuspend =
    this.edidData[SUPPORTED_FEATURES_BITMAP] & DPMS_SUSPEND ? true : false;
  var DPMS_ACTIVE_OFF = 0x20;
  bdp.dpmsActiveOff =
    this.edidData[SUPPORTED_FEATURES_BITMAP] & DPMS_ACTIVE_OFF ? true : false;
  var DISPLAY_TYPE_OFF = 3;
  var DISPLAY_TYPE_MASK = 0x03;
  bdp.displayType =
    (this.edidData[SUPPORTED_FEATURES_BITMAP] >> DISPLAY_TYPE_OFF) &
    DISPLAY_TYPE_MASK;

  var STANDARD_SRGB = 0x04;
  bdp.standardSRgb =
    this.edidData[SUPPORTED_FEATURES_BITMAP] & STANDARD_SRGB ? true : false;
  var PREFERRED_TIMING = 0x02;
  bdp.preferredTiming =
    this.edidData[SUPPORTED_FEATURES_BITMAP] & PREFERRED_TIMING ? true : false;
  var GTF_SUPPORTED = 0x01;
  bdp.gtfSupported =
    this.edidData[SUPPORTED_FEATURES_BITMAP] & GTF_SUPPORTED ? true : false;
  return bdp;
};
...
Edid.prototype.parseDtd = function (dtdIndex) {
  ...
  // Pixel Clock in MHz
  dtd.pixelClock =
    ((this.edidData[dtdIndex + 1] << 8) | this.edidData[dtdIndex]) / 100;

  var HOR_ACTIVE_OFF = 4;
  var HOR_ACTIVE_PIX_MASK = 0x0f;
  dtd.horActivePixels =
    (((this.edidData[dtdIndex + 4] >> HOR_ACTIVE_OFF) & HOR_ACTIVE_PIX_MASK) <<
      8) |
    this.edidData[dtdIndex + 2];

  var HOR_BLANK_MASK = 0x0f;
  dtd.horBlankPixels =
    ((this.edidData[dtdIndex + 4] & HOR_BLANK_MASK) << 8) |
    this.edidData[dtdIndex + 3];

  var VERT_ACTIVE_OFF = 4;
  var VERT_ACTIVE_MASK = 0x0f;
  dtd.vertActivePixels =
    (((this.edidData[dtdIndex + 7] >> VERT_ACTIVE_OFF) & VERT_ACTIVE_MASK) <<
      8) |
    this.edidData[dtdIndex + 5];
  var VERT_BLANK_MASK = 0x0f;
  dtd.vertBlankPixels =
    ((this.edidData[dtdIndex + 7] & VERT_BLANK_MASK) << 8) |
    this.edidData[dtdIndex + 6];

  var HOR_SYNC_OFF_OFF = 6;
  var HOR_SYNC_OFF_MASK = 0x03;
  dtd.horSyncOff =
    (((this.edidData[dtdIndex + 11] >> HOR_SYNC_OFF_OFF) & HOR_SYNC_OFF_MASK) <<
      8) |
  ...
  dtd.horSyncPulse =
    (((this.edidData[dtdIndex + 11] >> HOR_SYNC_PULSE_OFF) &
      HOR_SYNC_PULSE_MASK) <<
      8) |
  ...
  dtd.vertSyncOff =
    (((this.edidData[dtdIndex + 11] >> VERT_SYNC_OFF_TOP_OFF) &
  ...
  dtd.horDisplaySize =
    (((this.edidData[dtdIndex + 14] >> HOR_DISPLAY_TOP_OFF) &
  ...
  dtd.interlaced =
    this.edidData[dtdIndex + 17] & INTERLACED_MASK ? true : false;

  var STEREO_MODE_OFFSET = 5;
  var STEREO_MODE_MASK = 0x03;
  dtd.stereoMode =
    (this.edidData[dtdIndex + 17] >> STEREO_MODE_OFFSET) & STEREO_MODE_MASK;

  var SYNC_TYPE_OFFSET = 3;
  var SYNC_TYPE_MASK = 0x03;
  dtd.syncType =
    (this.edidData[dtdIndex + 17] >> SYNC_TYPE_OFFSET) & SYNC_TYPE_MASK;
  // Bit is dependent on sync type
  if (dtd.syncType == this.syncTypeEnum.DIGITAL_SEPARATE) {
    var VSYNC_POLARITY_MASK = 0x04;
    dtd.vSyncPolarity =
      this.edidData[dtdIndex + 17] & VSYNC_POLARITY_MASK ? true : false;
  } else {
    var VSYNC_SERRATED_MASK = 0x04;
    dtd.vSyncSerrated =
      this.edidData[dtdIndex + 17] & VSYNC_SERRATED_MASK ? true : false;
  }
  ...
  if (
    ...
    dtd.syncAllRGBLines =
      this.edidData[dtdIndex + 17] & SYNC_ALL_RGB_MASK ? true : false;
  } else {
    var HSYNC_POLARY_MASK = 0x02;
    dtd.hSyncPolarity =
      this.edidData[dtdIndex + 17] & HSYNC_POLARY_MASK ? true : false;
  }
  var TWO_WAY_STEREO_MASK = 0x01;
  dtd.twoWayStereo =
    this.edidData[dtdIndex + 17] & TWO_WAY_STEREO_MASK ? true : false;
...
Edid.prototype.parseDataBlockCollection = function (extIndex) {
  ...
  while (index < endAddress) {
    // Parse tag code
    var blockTagCode =
      (this.edidData[index] >> TAG_CODE_OFFSET) & TAG_CODE_MASK;
    // Parse Length
...
Edid.prototype.parseAudioDataBlock = function (startAddress, blockLength) {
  ...
  while (shortAudDescIndex < numberShortAudioDescriptors) {
    ...
    if (shortAudDesc.format <= this.audioFormatArray[0]) {
      ...
      shortAudDesc.audioFormatCode =
        this.edidData[index + 2] & AUDIO_FORMAT_CODE_MASK;
      ...
      shortAudDesc.formatCodeExt =
        (this.edidData[index + 2] >> FORMAT_CODE_EXT_OFF) &
        FORMAT_CODE_EXT_MASK;
    }
...
Edid.prototype.parseVendorDataBlockHDMI14 = function (
  startAddress,
  blockLength,
  vendorBlock
) {
  ...
  if (blockLength >= MAX_TMDS_CLOCK_ADDRESS) {
    ...
    vendorBlock.maxTmdsRate =
      this.edidData[vsdbAddress + MAX_TMDS_CLOCK_ADDRESS] * 5;
  }
  ...
  if (blockLength >= LATENCY_PRESENT_ADDRESS) {
    ...
    vendorBlock.latencyPresent =
      this.edidData[vsdbAddress + LATENCY_PRESENT_ADDRESS] &
      LATENCY_FIELDS_PRESENT_MASK
        ? true
        : false;
  ...
  if (vendorBlock.latencyPresent && blockLength >= AUDIO_LATENCY_ADDRESS) {
    ...
    vendorBlock.videoLatency =
      (this.edidData[vsdbAddress + VIDEO_LATENCY_ADDRESS] - 1) * 2;

    var AUDIO_LATENCY_ADDRESS = 10;
    // TODO: Add description
    vendorBlock.audioLatency =
      (this.edidData[vsdbAddress + AUDIO_LATENCY_ADDRESS] - 1) * 2;
  }
...
Edid.prototype.parseVendorDataBlockHDMI20 = function (
  startAddress,
  blockLength,
  vendorBlock
) {
  ...
  vendorBlock.supportsSCDC =
    this.edidData[vsdbAddress + FIELD_ADDRESS] & FIELD_MASK ? true : false;
...
Edid.prototype.parseVendorDataBlock = function (startAddress, blockLength) {
  ...
  if (vendorBlock.ieeeIdentifier == this.ieeeOuiType.HDMI14.value) {
    // HDMI 1.4 VSDB
    return this.parseVendorDataBlockHDMI14(
      startAddress,
      blockLength,
      vendorBlock
    );
    ...
    // HDMI 2.0 VSDB
    return this.parseVendorDataBlockHDMI20(
      startAddress,
      blockLength,
      vendorBlock
    );
...
Edid.prototype.parseVideoCapabilityDataBlock = function (
  startAddress,
  blockLength,
  extendedTagBlock
) {
  ...
  extendedTagBlock.quantizationRangeYCC =
    this.edidData[startAddress + FIELD_ADDRESS] & FIELD_MASK ? true : false;
  ...
  FIELD_SHIFT = 4;
  fieldData =
    (this.edidData[startAddress + FIELD_ADDRESS] & FIELD_MASK) >> FIELD_SHIFT;
...
Edid.prototype.parseColorimetryDataBlock = function (
  startAddress,
  blockLength,
  extendedTagBlock
) {
...
Edid.prototype.parseYCbCr420VideoDataBlock = function (
  startAddress,
  blockLength,
  extendedTagBlock
) {
...
Edid.prototype.parseYCbCr420CapabilityMapDataBlock = function (
  startAddress,
  blockLength,
  extendedTagBlock
) {
  extendedTagBlock.extendedTag =
    this.extendedDataBlockType.YCBCR420_CAPABILITY_MAP;
    ...
    for (FIELD_MASK = 0x01; FIELD_MASK <= 0x80; FIELD_MASK <<= 1) {
      ...
      if (YCbCr420Capable) {
        extendedTagBlock.YCbCr420CapableShortVideoDescriptors[
          YCbCr420svdIndex
        ] = this.videoBlock.shortVideoDescriptors[svdIndex];
        YCbCr420svdIndex++;
      }
      svdIndex++;
    }
...
Edid.prototype.parseExtendedTagDataBlock = function (
  startAddress,
  blockLength
) {
  ...
  if (
    extendedBlockTagCode == this.extendedDataBlockType.VIDEO_CAPABILITY.value
  ) {
    return this.parseVideoCapabilityDataBlock(
      startAddress,
      blockLength,
      extendedTagBlock
    );
  } else if (
    extendedBlockTagCode == this.extendedDataBlockType.COLORIMETRY.value
  ) {
    return this.parseColorimetryDataBlock(
      startAddress,
      blockLength,
      extendedTagBlock
    );
  } else if (
    extendedBlockTagCode == this.extendedDataBlockType.YCBCR420_VIDEO.value
  ) {
    return this.parseYCbCr420VideoDataBlock(
      startAddress,
      blockLength,
      extendedTagBlock
    );
    ...
    this.extendedDataBlockType.YCBCR420_CAPABILITY_MAP.value
  ) {
    ...
    extendedTagBlock.extendedTag =
      this.edidData[startAddress + EXTENDED_TAG_ADDRESS];
  }
